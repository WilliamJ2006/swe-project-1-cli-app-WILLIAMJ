...
class ShoppingCart {
    ...
    addItem() {
        ...
        while (this.shoppingCart.find((item) => item.name === name)) {
        ...
        if (this.running) {
            ...
            this.shoppingCart.push({ name: name, quantity: quantity, price: price });
        } else {
    ...
    removeItem() {
        ...
        this.shoppingCart = this.shoppingCart.filter(item => item.quantity > 0);
    ...
    viewList() {
        if (this.shoppingCart.length > 0) {
            ...
            this.shoppingCart.forEach((item) => console.log(`- ${item.quantity} ${item.name}: $${(item.quantity * item.price).toFixed(2)}`));
            console.log(`\nTotal Items: ${this.shoppingCart.reduce((acc, curr) => acc + curr.quantity, 0)}`)
            console.log(`Total Price: $${this.shoppingCart.reduce((acc, curr) => acc + (curr.quantity * curr.price), 0).toFixed(2)}`)
        } else {
